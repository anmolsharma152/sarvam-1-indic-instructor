...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune Sarvam-1 on Job Descriptions"
    )
    parser.add_argument(
        "--model_id",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="models/sarvam-job-desc-lora",
        help="Dir to save adapter checkpoint",
    )
    parser.add_argument(
        "--dry-run",
        dest="dry_run",
        action="store_true",
        help="Run a fast, tiny training loop on CPU for testing",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--wandb_project",
        type=str,
        default="sarvam-job-desc-finetuning",
    ...
    if args.dry_run or device == "cpu":
        ...
        print(
            "Using tiny random model and minimal data steps to verify pipeline execution."
        )
        print("=" * 80 + "\n")
    ...
    if args.dry_run or device == "cpu":
        ...
        dataset["validation"] = dataset["validation"].select(
            range(min(5, len(dataset["validation"])))
        )

    print(
        f"Dataset loaded. Train size: {len(dataset['train'])}, Val size: {len(dataset['validation'])}"
    )
    ...
    if use_unsloth:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            model_id, quantization_config=None, device_map=None, trust_remote_code=True
        )
    ...
    training_args = SFTConfig(
        ...
        use_cpu=(device == "cpu"),
        report_to="wandb",
        run_name=f"sarvam-job-desc-{device}"
        if not args.dry_run
        else "sarvam-job-desc-dry-run",
        logging_dir="./logs",
    ...
    trainer = SFTTrainer(
        ...
        if use_unsloth
        else None,  # Unsloth prefers tokenizer passed here
